...
namespace HSL.Core
{
    ...
    public class ServerInstance : INotifyPropertyChanged, IDisposable
    {
        ...
        public class ResourceMeta : INotifyPropertyChanged
        {
        ...
        private void _watcherHandler(object sender, FileSystemEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    // Check if the change is in the resources directory
                    if (e.FullPath.StartsWith(ResourceDirectory, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (match.Success && match.Groups.Count > 1)
                        {
                            ...
                            if (e.ChangeType == WatcherChangeTypes.Created ||
                                e.ChangeType == WatcherChangeTypes.Changed)
                            {
                                if (File.Exists(Path.Combine(ResourceDirectory, resourceName, "meta.xml")))
                                {
                                    RefreshServerInformation();
                                    if (AutoReloadResources)
                                    {
                            ...
                            else if (e.ChangeType == WatcherChangeTypes.Deleted)
                            {
                                if (ResourceMap.ContainsKey(resourceName))
                                {
                                    ...
                                    Resources.RemoveAll(r => r.Name == resourceName);
                                    OnPropertyChanged(nameof(Resources));
                                }
                            }
                        }
                    }
                    else if (e.FullPath == ServerSettingsFile)
                    {
                        ...
                        {
                            ServerSettings._wasUpdated = false;
                        }
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                Trace.WriteLine($"Error in file watcher: {ex.Message}");
            }
        ...
        {
            if (SendInput("resrestart " + name) && ResourceMap.ContainsKey(name))
            {
        ...
        {
            if (SendInput("resstart " + name) && ResourceMap.ContainsKey(name))
            {
                ResourceMap[name].IsEnabled = true;
                OnPropertyChanged(nameof(ResourceMap));
            }
        ...
        {
            if (SendInput("resstop " + name) && ResourceMap.ContainsKey(name))
            {
